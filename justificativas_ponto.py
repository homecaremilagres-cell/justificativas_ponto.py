...
if botao_enviar:
    if colaborador == "Selecione...":
        ...
        try:
            dados_existentes = conn.read(ttl=0) # ttl=0 garante que ele busque o dado mais recente sem usar cache
        except Exception:
            # Se a planilha estiver totalmente vazia (sem cabeçalho ainda), cria um DataFrame vazio
